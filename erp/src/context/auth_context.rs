    ...
    #[derive(Default)]
    pub struct Cache {
        pub authenticate: Option<LoginSessionSql>,
    }
    #[async_trait]
    #[async_trait]
    pub trait AuthContextTrait {
        fn new(headers: HeaderMap,db: Arc<DatabaseConnection>,ip_address:Option<String>) -> Self;
        async fn must_anonymous(&self) -> Result<(), AuthError>;
        async fn try_authenticate(&self) -> Result<LoginSessionSql,AuthError> ;
        async fn must_authenticate(&self) -> Result<LoginSessionSql,AuthError> ;
    }
    #[async_trait]
